...
@section('content')
	...
	<!-- ============================================================== -->
			...
			@endif
             ...
             <a style="float:left; margin:5px;" href="{{ route('pekerjaan.create') }}" ><button class="btn btn-info">Tambah</button></a>
					<br>
              ...
              <div class="table-responsive" style="margin:20px;">
			   ...
			   @if(!isset($hanya_saya))
                   <select name="tahun" onchange="location = this.value;">
                      ...
                      ?>
                      <option value="pekerjaan?thn={{$i}}">{{$i}}</option>
                      <?php } ?>
                ...
                <table style="font-size:10px;" class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
				  ...
				  if(isset($cari)){
					if($cari == 'nego'){
						$pekerjaan=DB::table('laporan_pekerjaan')->where([['tahun',$tahundashboard],['status','Permintaan Penawaran Harga'],])->orWhere([['tahun',$tahundashboard],['status','Undangan Untuk Negosiasi'],])->orWhere([['tahun',$tahundashboard],['status','Berita Acara Atas Negosiasi'],])->orWhere([['tahun',$tahundashboard],['status','Penawaran Harga'],])->get();
					}else if($cari == 'berlangsung'){
						$pekerjaan=DB::table('laporan_pekerjaan')->where([['tahun',$tahundashboard],['status','Perintah Untuk Melaksanakan Pekerjaan'],])->get();	
					}else if($cari == 'selesai'){
						$pekerjaan=DB::table('laporan_pekerjaan')->where([['tahun',$tahundashboard],['status','Telah Melaporkan Selesainya Pekerjaan'],])->orWhere([['tahun',$tahundashboard],['status','Pemeriksaan Pekerjaan'],])->orWhere([['tahun',$tahundashboard],['status','Serah Terima Pekerjaan'],])->get();	
					}else if($cari == 'penagihan'){
						$pekerjaan=DB::table('laporan_pekerjaan')->where([['tahun',$tahundashboard],['status','Sudah Menyampaikan Penagihan'],])->get();	
					}else if($cari == 'onprogress'){
						$pekerjaan=DB::table('laporan_pekerjaan')->where([['tahun',$tahundashboard],['status','On Progress'],])->get();	
					}else if($cari == 'paid'){
						$pekerjaan=DB::table('laporan_pekerjaan')->where([['tahun',$tahundashboard],['status','Paid'],])->get();	
					}
				  }else if(isset($_GET["thn"])){
				  $pekerjaan=DB::table('laporan_pekerjaan')->where('tahun','=',$_GET["thn"])->get();
				  }else{
				  ...
				  ?>
				    ...
				    @if(isset($hanya_saya))
                      ...
                      <td><a href="javascript:" onclick="document.getElementById('dbln{{$no}}').submit()"><div style="width:100%;">{{$pic2->name}}</div></a></td>
                       <td><a href="javascript:" onclick="document.getElementById('dbln{{$no}}').submit()"><div style="width:100%;">
                          <?php
                           $spk=DB::table('dokumen_laporan_pekerjaan')->where([['id_laporan_pekerjaan',$pkn->id],['jenis_dokumen','SPK']])->get()->first();
					  ...
					  <td><a href="javascript:" onclick="document.getElementById('dbln{{$no}}').submit()"><div style="width:100%;">
					  <?php
					  $nilai_spk=DB::table('dokumen_laporan_pekerjaan')->where([['id_laporan_pekerjaan',$pkn->id],['jenis_dokumen','SPK'],])->sum('harga');
					  ...
					  <td><a href="javascript:" onclick="document.getElementById('dbln{{$no}}').submit()"><div style="width:100%;">{{$pkn->wilayah_operasi}}</div></a></td>
						<?php
							$jenis_pekerjaan=DB::table('jenis_pekerjaan')->where('id',$pkn->jenis_pekerjaan)->get()->first();
						?>
					  <td><a href="javascript:" onclick="document.getElementById('dbln{{$no}}').submit()"><div style="width:100%;">{{$jenis_pekerjaan->jenis_pekerjaan}}</div></a></td>
					...
					<td style="border: 0px;">
					<a style="float:left; margin-right:5px;" href="{{ route('pekerjaan.edit',$pkn->id) }}"><i class="material-icons">create</i></a>
					</td>
					...
					<form style="display:none;" id="delete{{$no}}" method="post" action="{{ route('pekerjaan.destroy',$pkn->id) }}">
