...
@section('content')
	...
	<div class="row">
		<div class="col-md-12">
			<!-- content -->
			<h2>Edit Data</h2>
			<form class="form-line" action="<?php echo url('/edit_dokumen_pekerjaan_proses_pagu'); ?>" method="post" enctype="multipart/form-data">
				{{ csrf_field() }}
				
				<?php $pkn = DB::table('dokumen_laporan_pekerjaan_pagu')->where('id', '=', $id)->get()->first(); ?>
				...
				<div style="<?php if($pkn->jenis_dokumen=="LPS" or $pkn->jenis_dokumen=="BAP" or $pkn->jenis_dokumen=="BAST"){echo "";}else{echo "display:none;";}?>" id="perpem" class="form-line">
					...
					$pp=DB::table('spk_dev_pagu')->where('id',$pkn->id_spk_dev)->get()->first();
					if($pp){
					?>
